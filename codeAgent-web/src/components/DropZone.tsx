...
export const DropZone = ({ onFileSelect, onGitClone, onServerConnect }: DropZoneProps) => {
  ...
  const [serverUrl, setServerUrl] = useState(() =>
    localStorage.getItem('codeAgent-server-url') || ''
  );
  ...
  const handleServerConnect = async () => {
    ...
    localStorage.setItem('codeAgent-server-url', serverUrl);
      ...
      <div className="relative w-full max-w-lg">
        ...
        {activeTab === 'server' && (
          <div className="p-8 bg-surface border border-border-default rounded-3xl">
            ...
            <p className="text-sm text-text-secondary text-center mb-6">
              Load a pre-built knowledge graph from a running codeAgent server
            </p>
