...
- Renamed all internal paths from `kuzu` to `lbug` (storage: `.codeAgent/kuzu` → `.codeAgent/lbug`)
...
- **Optional skill generation** via `npx codeAgent analyze --skills` — generates AI agent skills from KuzuDB knowledge graph (#171) — @zander-raycraft
...
- Auto-reindex hook that runs `codeAgent analyze` after commits and merges, with automatic embeddings preservation (#205) — @L1nusB
...
- **Lazy CLI module loading**: All CLI subcommands now use `createLazyAction()` to defer heavy imports (tree-sitter, ONNX, KuzuDB) until invocation, significantly improving `codeAgent mcp` startup time (#207)
