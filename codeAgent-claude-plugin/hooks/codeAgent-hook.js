#!/usr/bin/env node
/**
 * codeAgent Claude Code Plugin Hook
 *
 * PreToolUse  — intercepts Grep/Glob/Bash searches and augments
 *               with graph context from the codeAgent index.
...
/**
 * Find the .codeAgent directory by walking up from startDir.
 * Returns the path to .codeAgent/ or null if not found.
 */
function findcodeAgentDir(startDir) {
  let dir = startDir || process.cwd();
  for (let i = 0; i < 5; i++) {
    const candidate = path.join(dir, '.codeAgent');
...
/**
 * Spawn a codeAgent CLI command synchronously.
 * Detects binary on PATH once, then runs exactly once.
 *
 * SECURITY: Never use shell: true with user-controlled arguments.
 * On Windows, invoke codeAgent.cmd directly (no shell needed).
 */
function runcodeAgentCli(args, cwd, timeout) {
  const isWin = process.platform === 'win32';

  // Detect whether 'codeAgent' is on PATH (cheap check, no execution)
  let useDirectBinary = false;
  try {
    const which = spawnSync(
      isWin ? 'where' : 'which', ['codeAgent'],
...
function handlePreToolUse(input) {
  ...
  if (!findcodeAgentDir(cwd)) return;
  ...
  let result = '';
  try {
    const child = runcodeAgentCli(['augment', '--', pattern], cwd, 7000);
 ...
 *
 * Instead of spawning a full `codeAgent analyze` synchronously (which blocks
 * the agent for up to 120s and risks LadybugDB corruption on timeout), we do a
 * lightweight staleness check: compare `git rev-parse HEAD` against the
 * lastCommit stored in `.codeAgent/meta.json`. If they differ, notify the
...
function handlePostToolUse(input) {
  ...
  const cwd = input.cwd || process.cwd();
  if (!path.isAbsolute(cwd)) return;
  const codeAgentDir = findcodeAgentDir(cwd);
  if (!codeAgentDir) return;
  ...
  try {
    const meta = JSON.parse(fs.readFileSync(path.join(codeAgentDir, 'meta.json'), 'utf-8'));
  ...
  const analyzeCmd = `npx codeAgent analyze${hadEmbeddings ? ' --embeddings' : ''}`;
  sendHookResponse('PostToolUse',
    `codeAgent index is stale (last indexed: ${lastCommit ? lastCommit.slice(0, 7) : 'never'}). ` +
...
function main() {
  try {
    ...
    if (process.env.codeAgent_DEBUG) {
      console.error('codeAgent hook error:', (err.message || '').slice(0, 200));
    }
  }
}
