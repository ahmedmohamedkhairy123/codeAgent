#!/bin/bash
# codeAgent beforeShellExecution hook for Cursor
# Receives JSON on stdin with { command, cwd, timeout }
# Returns JSON on stdout with { permission, agent_message }
#
# Extracts search pattern from grep/rg commands, runs codeAgent augment,
...
# Run codeAgent augment
RESULT=$(npx -y codeAgent augment "$PATTERN" 2>/dev/null)
