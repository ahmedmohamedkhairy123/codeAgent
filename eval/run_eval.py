#!/usr/bin/env python3
"""
codeAgent SWE-bench Evaluation Runner

Main entry point for running SWE-bench evaluations with and without codeAgent.
Supports running a single configuration or a full matrix of models x modes.

Usage:
    # Single run (default: native_augment mode — codeAgent tools + grep enrichment)
...
logger = logging.getLogger("codeAgent_eval")
console = Console()
...
def process_instance(
    ...
    try:
        ...
        if env_class_name == "eval.environments.codeAgent_docker.codeAgentDockerEnvironment":
            from environments.codeAgent_docker import codeAgentDockerEnvironment
            env_config["image"] = get_swebench_docker_image(instance)
            env = codeAgentDockerEnvironment(**env_config)
        else:
        ...
        agent_class_name = agent_config.pop("agent_class", "eval.agents.codeAgent_agent.codeAgentAgent")

        from agents.codeAgent_agent import codeAgentAgent
        traj_path = instance_dir / f"{instance_id}.traj.json"
        agent_config["output_path"] = traj_path
        agent = codeAgentAgent(model, env, **agent_config)
        ...
        result["codeAgent_metrics"] = agent.codeAgent_metrics.to_dict()
...
def list_configs():
    ...
    for name in AVAILABLE_MODES:
        ...
        gn_mode = config.get("agent", {}).get("codeAgent_mode", "baseline")
        console.print(f"  {name:<20} codeAgent_mode={gn_mode}")
...
def _print_summary(results: list[dict], model: str, mode: str):
    ...
    # codeAgent-specific metrics
    gn_tool_calls = sum(
        r.get("codeAgent_metrics", {}).get("total_tool_calls", 0) for r in results
    )
    gn_augment_hits = sum(
        r.get("codeAgent_metrics", {}).get("augmentation_hits", 0) for r in results
    )
    if gn_tool_calls > 0:
        table.add_row("codeAgent Tool Calls", str(gn_tool_calls))
...
def _print_matrix_summary(all_results: dict[str, list[dict]]):
    ...
    for run_id, results in sorted(all_results.items()):
        ...
        gn_calls = sum(r.get("codeAgent_metrics", {}).get("total_tool_calls", 0) for r in results)
