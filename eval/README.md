...
Evaluate whether codeAgent code intelligence improves AI agent performance on real software engineering tasks. Runs SWE-bench instances across multiple models and compares baseline (no graph) vs codeAgent-enhanced configurations.
...
| `native` | Baseline + explicit codeAgent tools via eval-server (~100ms) |
| `native_augment` | Native tools + grep results automatically enriched with graph context (**recommended**) |

> **Recommended**: Use `native_augment` mode. It mirrors the Claude Code model — the agent gets both explicit codeAgent tools (fast bash commands) AND automatic enrichment of grep results with callers, callees, and execution flows. The agent decides when to use explicit tools vs rely on enriched search output.
...
# Baseline comparison (no codeAgent)
...
# codeAgent tool usage analysis
python -m analysis.analyze_results codeAgent-usage results/
...
eval/
  ...
  agents/
    codeAgent_agent.py      # codeAgentAgent: extends DefaultAgent with augmentation + metrics
  environments/
    codeAgent_docker.py     # Docker env with codeAgent + eval-server + standalone tool scripts
  bridge/
    codeAgent_tools.sh      # Bash wrappers (legacy — now standalone scripts are installed directly)
  ...
  prompts/
    ...
    system_native.jinja            # System: + codeAgent tool reference
    instance_native.jinja          # Instance: + codeAgent debugging workflow
    system_native_augment.jinja    # System: + codeAgent tools + grep enrichment docs
    instance_native_augment.jinja  # Instance: + codeAgent workflow + risk assessment
  configs/
...
2. **codeAgent setup**: Node.js + codeAgent installed, `codeAgent analyze` runs (or restores from cache)
3. **Eval-server starts**: `codeAgent eval-server` daemon (persistent HTTP server, keeps LadybugDB warm)
4. **Standalone tool scripts installed** in `/usr/local/bin/` — works with `subprocess.run` (no `.bashrc` needed)
5. Agent runs with the configured model + system prompt + codeAgent tools
6. Agent's patch is extracted as a git diff
7. Metrics collected: cost, tokens, tool calls, codeAgent usage, augmentation stats

### Tool architecture

```
Agent → bash command → /usr/local/bin/codeAgent-query
  → curl localhost:4848/tool/query     (fast path: eval-server, ~100ms)
  → npx codeAgent query                 (fallback: cold CLI, ~5-10s)
```
...
SWE-bench repos repeat (Django has 200+ instances at different commits). The harness caches codeAgent indexes per `(repo, commit)` hash in `~/.codeAgent-eval-cache/` to avoid redundant re-indexing.

### Grep augmentation (native_augment mode)

When the agent runs `grep` or `rg`, the observation is post-processed: the agent class calls `codeAgent-augment` on the search pattern and appends `[codeAgent]` annotations showing callers, callees, and execution flows for matched symbols. This mirrors the Claude Code / Cursor hook integration.
...
| GN Tool Calls | How many codeAgent tools the agent used |
