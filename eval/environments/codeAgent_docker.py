"""
codeAgent Docker Environment for SWE-bench Evaluation

Extends mini-swe-agent's Docker environment to:
1. Install codeAgent (Node.js + npm + codeAgent package)
2. Run `codeAgent analyze` on the repository
...
Architecture:
  Agent bash cmd → /usr/local/bin/codeAgent-query → curl localhost:4848/tool/query → eval-server → KuzuDB
  Fallback: → npx codeAgent query (cold start, slower)
...
logger = logging.getLogger("codeAgent_docker")

DEFAULT_CACHE_DIR = Path.home() / ".codeAgent-eval-cache"
...
PORT="${codeAgent_EVAL_PORT:-__PORT__}"
query="$1"; task_ctx="${2:-}"; goal="${3:-}"
[ -z "$query" ] && echo "Usage: codeAgent-query <query> [task_context] [goal]" && exit 1
args="{\"query\": \"$query\""
...
cd /testbed && npx codeAgent query "$query" 2>&1
'''

TOOL_SCRIPT_CONTEXT = r'''#!/bin/bash
PORT="${codeAgent_EVAL_PORT:-__PORT__}"
name="$1"; file_path="${2:-}"
[ -z "$name" ] && echo "Usage: codeAgent-context <symbol_name> [file_path]" && exit 1
args="{\"name\": \"$name\""
...
if [ $? -eq 0 ] && [ -n "$result" ]; then echo "$result"; exit 0; fi
cd /testbed && npx codeAgent context "$name" 2>&1
'''

TOOL_SCRIPT_IMPACT = r'''#!/bin/bash
PORT="${codeAgent_EVAL_PORT:-__PORT__}"
target="$1"; direction="${2:-upstream}"
[ -z "$target" ] && echo "Usage: codeAgent-impact <symbol_name> [upstream|downstream]" && exit 1
...
if [ $? -eq 0 ] && [ -n "$result" ]; then echo "$result"; exit 0; fi
cd /testbed && npx codeAgent impact "$target" --direction "$direction" 2>&1
'''

TOOL_SCRIPT_CYPHER = r'''#!/bin/bash
PORT="${codeAgent_EVAL_PORT:-__PORT__}"
query="$1"
[ -z "$query" ] && echo "Usage: codeAgent-cypher <cypher_query>" && exit 1
result=$(curl -sf -X POST "http://127.0.0.1:${PORT}/tool/cypher" -H "Content-Type: application/json" -d "{\"query\": \"$query\"}" 2>/dev/null)
if [ $? -eq 0 ] && [ -n "$result" ]; then echo "$result"; exit 0; fi
cd /testbed && npx codeAgent cypher "$query" 2>&1
'''

TOOL_SCRIPT_AUGMENT = r'''#!/bin/bash
cd /testbed && npx codeAgent augment "$1" 2>&1 || true
'''
...
if [ $? -eq 0 ] && [ -n "$result" ]; then echo "$result"; exit 0; fi
cd /testbed && npx codeAgent list 2>&1
'''


class codeAgentDockerEnvironment(DockerEnvironment):
    """
    Docker environment with codeAgent pre-installed, indexed, and eval-server running.

    Setup flow:
    1. Start Docker container (base SWE-bench image)
    2. Install Node.js + codeAgent inside the container
    3. Run `codeAgent analyze` (or restore from cache)
    4. Start `codeAgent eval-server` daemon (keeps KuzuDB warm)
    5. Install standalone tool scripts in /usr/local/bin/
    6. Agent runs with near-instant codeAgent tool calls
    """

    def __init__(
        self,
        *,
        enable_codeAgent: bool = True,
        cache_dir: str | Path | None = None,
        skip_embeddings: bool = True,
        codeAgent_timeout: int = 120,
        eval_server_port: int = EVAL_SERVER_PORT,
        **kwargs,
    ):
        super().__init__(**kwargs)
        self.enable_codeAgent = enable_codeAgent
        self.cache_dir = Path(cache_dir) if cache_dir else DEFAULT_CACHE_DIR
        self.skip_embeddings = skip_embeddings
        self.codeAgent_timeout = codeAgent_timeout
        self.eval_server_port = eval_server_port
        self.index_time: float = 0.0
        self._codeAgent_ready = False

    def start(self) -> dict:
        """Start the container and set up codeAgent."""
        result = super().start()

        if self.enable_codeAgent:
            try:
                self._setup_codeAgent()
            except Exception as e:
                logger.warning(f"codeAgent setup failed, continuing without it: {e}")
                self._codeAgent_ready = False

        return result

    def _setup_codeAgent(self):
        """Install and configure codeAgent in the container."""
        start = time.time()

        self._ensure_nodejs()
        self._install_codeAgent()
        self._index_repository()
        self._start_eval_server()
        self._install_tools()

        self.index_time = time.time() - start
        self._codeAgent_ready = True
        logger.info(f"codeAgent setup completed in {self.index_time:.1f}s")
    ...
    def _install_codeAgent(self):
        """Install the codeAgent npm package globally."""
        check = self.execute({"command": "npx codeAgent --version 2>/dev/null || echo 'NOT_FOUND'"})
        if "NOT_FOUND" in check.get("output", ""):
            logger.info("Installing codeAgent...")
            result = self.execute({
                "command": "npm install -g codeAgent",
                "timeout": 60,
            })
            if result.get("returncode", 1) != 0:
                raise RuntimeError(f"Failed to install codeAgent: {result.get('output', '')}")

    def _index_repository(self):
        """Run codeAgent analyze on the repo, using cache if available."""
        repo_info = self._get_repo_info()
        cache_key = self._make_cache_key(repo_info)
        cache_path = self.cache_dir / cache_key

        if cache_path.exists():
            logger.info(f"Restoring codeAgent index from cache: {cache_key}")
            self._restore_cache(cache_path)
            return

        logger.info("Running codeAgent analyze...")
        skip_flag = "--skip-embeddings" if self.skip_embeddings else ""
        result = self.execute({
            "command": f"cd /testbed && npx codeAgent analyze . {skip_flag} 2>&1",
            "timeout": self.codeAgent_timeout,
        })

        if result.get("returncode", 1) != 0:
            output = result.get("output", "")
            if "error" in output.lower() and "indexed" not in output.lower():
                raise RuntimeError(f"codeAgent analyze failed: {output[-500:]}")

        self._save_cache(cache_path, repo_info)

    def _start_eval_server(self):
        """Start the codeAgent eval-server daemon in the background."""
        logger.info(f"Starting eval-server on port {self.eval_server_port}...")

        self.execute({
            "command": (
                f"nohup npx codeAgent eval-server --port {self.eval_server_port} "
                f"--idle-timeout 600 "
                f"> /tmp/codeAgent-eval-server.log 2>&1 &"
            ),
            "timeout": 5,
        })
        ...
        log_output = self.execute({
            "command": "cat /tmp/codeAgent-eval-server.log 2>/dev/null | tail -20",
        })
        logger.warning(
    ...
    def _install_tools(self):
        """
        Install standalone codeAgent tool scripts in /usr/local/bin/.
        ...
        tools = {
            "codeAgent-query": TOOL_SCRIPT_QUERY,
            "codeAgent-context": TOOL_SCRIPT_CONTEXT,
            "codeAgent-impact": TOOL_SCRIPT_IMPACT,
            "codeAgent-cypher": TOOL_SCRIPT_CYPHER,
            "codeAgent-augment": TOOL_SCRIPT_AUGMENT,
            "codeAgent-overview": TOOL_SCRIPT_OVERVIEW,
        }
        ...
        for name, script in tools.items():
            ...
            self.execute({
                "command": f"cat << 'codeAgent_SCRIPT_EOF' > /usr/local/bin/{name}\n{script_content}\ncodeAgent_SCRIPT_EOF\nchmod +x /usr/local/bin/{name}",
                "timeout": 5,
            })

        logger.info(f"Installed {len(tools)} codeAgent tool scripts in /usr/local/bin/")
    ...
    def _save_cache(self, cache_path: Path, repo_info: dict):
        """Save the codeAgent index to the host cache directory."""
        try:
            cache_path.mkdir(parents=True, exist_ok=True)

            find_result = self.execute({
                "command": "find /root/.codeAgent -name 'kuzu' -type d 2>/dev/null | head -1"
            })
            codeAgent_dir = find_result.get("output", "").strip()

            if codeAgent_dir:
                parent = str(Path(codeAgent_dir).parent)
                self.execute({
                    "command": f"cd {parent} && tar czf /tmp/codeAgent-cache.tar.gz .",
                    "timeout": 30,
                })
                ...
                if container_id:
                    ...
                    sp.run(
                        ["docker", "cp", f"{container_id}:/tmp/codeAgent-cache.tar.gz",
                         str(cache_path / "index.tar.gz")],
                        check=True, capture_output=True,
                    )
                    (cache_path / "metadata.json").write_text(json.dumps(repo_info, indent=2))
                    logger.info(f"Cached codeAgent index: {cache_path}")

        except Exception as e:
            logger.warning(f"Failed to cache codeAgent index: {e}")
            if cache_path.exists():
                shutil.rmtree(cache_path, ignore_errors=True)

    def _restore_cache(self, cache_path: Path):
        """Restore a cached codeAgent index into the container."""
        try:
            ...
            if container_id:
                ...
                self.execute({"command": "mkdir -p /root/.codeAgent"})

                storage_result = self.execute({
                    "command": "npx codeAgent list 2>/dev/null | grep -o '/root/.codeAgent/[^ ]*' | head -1 || echo '/root/.codeAgent/repos/default'"
                })
                storage_path = storage_result.get("output", "").strip() or "/root/.codeAgent/repos/default"
                self.execute({"command": f"mkdir -p {storage_path}"})

                sp.run(
                    ["docker", "cp", str(cache_tarball), f"{container_id}:/tmp/codeAgent-cache.tar.gz"],
                    check=True, capture_output=True,
                )
                self.execute({
                    "command": f"cd {storage_path} && tar xzf /tmp/codeAgent-cache.tar.gz",
                    "timeout": 30,
                })
                logger.info("codeAgent index restored from cache")
    ...
    def get_template_vars(self) -> dict:
        """Add codeAgent-specific template variables."""
        base_vars = super().get_template_vars()
        base_vars["codeAgent_ready"] = self._codeAgent_ready
        base_vars["codeAgent_index_time"] = self.index_time
        return base_vars

    def serialize(self) -> dict:
        """Include codeAgent environment info in serialization."""
        base = super().serialize()
        base.setdefault("info", {})["codeAgent_env"] = {
            "enabled": self.enable_codeAgent,
            "ready": self._codeAgent_ready,
