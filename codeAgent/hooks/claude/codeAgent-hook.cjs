...
/**
 * Resolve the codeAgent CLI path.
 * 1. Relative path (works when script is inside npm package)
 * 2. require.resolve (works when codeAgent is globally installed)
...
function resolveCliPath() {
  ...
  if (!fs.existsSync(cliPath)) {
    try {
      cliPath = require.resolve('codeAgent/dist/cli/index.js');
    } catch {
      cliPath = '';
    }
  }
 ...
 */
function runcodeAgentCli(cliPath, args, cwd, timeout) {
...
function handlePreToolUse(input) {
  ...
  let result = '';
  try {
    const child = runcodeAgentCli(cliPath, ['augment', '--', pattern], cwd, 7000);
