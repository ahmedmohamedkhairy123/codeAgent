 ...
 * HTTP API Server
 *
 * REST API for browser-based clients to query the local .codeAgent/ index.
...
export const createServer = async (port: number, host: string = '127.0.0.1') => {
  ...
  app.use(cors({
    origin: (origin, callback) => {
      if (
        ...
        || origin === 'https://codeAgent.vercel.app'
      ) {
        callback(null, true);
      } else {
  ...
  app.get('/api/repo', async (req, res) => {
    try {
      ...
      if (!entry) {
        res.status(404).json({ error: 'Repository not found. Run: codeAgent analyze' });
        return;
      }
  ...
  const server = app.listen(port, host, () => {
    console.log(`codeAgent server running on http://${host}:${port}`);
  });
