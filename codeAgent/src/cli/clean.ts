 ...
 * Clean Command
 * 
 * Removes the .codeAgent index from the current repository.
...
export const cleanCommand = async (options?: { force?: boolean; all?: boolean }) => {
  ...
  if (options?.all) {
    if (!options?.force) {
      ...
      console.log(`This will delete codeAgent indexes for ${entries.length} repo(s):`);
  ...
  if (!options?.force) {
    console.log(`This will delete the codeAgent index for: ${repoName}`);
