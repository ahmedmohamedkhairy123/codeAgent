 ...
 * so the codeAgent MCP server is available in all projects.
 */
...
function getMcpEntry() {
  if (process.platform === 'win32') {
    return {
      command: 'cmd',
      args: ['/c', 'npx', '-y', 'codeAgent@latest', 'mcp'],
    };
  }
  return {
    command: 'npx',
...
/**
 * Merge codeAgent entry into an existing MCP config JSON object.
...
function mergeMcpConfig(existing: any): any {
  ...
  existing.mcpServers.codeAgent = getMcpEntry();
  return existing;
}
...
async function setupClaudeCode(result: SetupResult): Promise<void> {
  ...
  console.log('');
  console.log('  Claude Code detected. Run this command to add codeAgent MCP:');
  console.log('');
  console.log('    claude mcp add codeAgent -- npx -y codeAgent mcp');
  console.log('');
  result.configured.push('Claude Code (MCP manual step printed)');
}

/**
 * Install codeAgent skills to ~/.claude/skills/ for Claude Code.
 */
...
/**
 * Install codeAgent hooks to ~/.claude/settings.json for Claude Code.
...
async function installClaudeCodeHooks(result: SetupResult): Promise<void> {
  ...
  // Source hooks bundled within the codeAgent package (hooks/claude/)
  const pluginHooksPath = path.join(__dirname, '..', '..', 'hooks', 'claude');

  // Copy unified hook script to ~/.claude/hooks/codeAgent/
  const destHooksDir = path.join(claudeDir, 'hooks', 'codeAgent');

  try {
    await fs.mkdir(destHooksDir, { recursive: true });

    const src = path.join(pluginHooksPath, 'codeAgent-hook.cjs');
    const dest = path.join(destHooksDir, 'codeAgent-hook.cjs');
    try {
    ...
    const hookPath = path.join(destHooksDir, 'codeAgent-hook.cjs').replace(/\\/g, '/');
    ...
    // Helper: add a hook entry if one with 'codeAgent-hook' isn't already registered
    ...
    function ensureHookEntry(
      ...
      const hasHook = existing.hooks[eventName].some(
        (h: HookEntry) => h.hooks?.some(hh => hh.command?.includes('codeAgent-hook'))
      );
      if (!hasHook) {
    ...
    ensureHookEntry('PreToolUse', 'Grep|Glob|Bash', 10, 'Enriching with codeAgent graph context...');
    ensureHookEntry('PostToolUse', 'Bash', 10, 'Checking codeAgent index freshness...');
...
async function setupOpenCode(result: SetupResult): Promise<void> {
  ...
  try {
    ...
    config.mcp.codeAgent = getMcpEntry();
...
function getCodexMcpTomlSection(): string {
  ...
  return `[mcp_servers.codeAgent]\ncommand = ${command}\nargs = ${args}\n`;
}

/**
 * Append codeAgent MCP server config to Codex's config.toml if missing.
 */
async function upsertCodexConfigToml(configPath: string): Promise<void> {
  ...
  if (existing.includes('[mcp_servers.codeAgent]')) {
    return;
  }
...
async function setupCodex(result: SetupResult): Promise<void> {
  ...
  try {
    ...
    await execFileAsync(
      'codex',
      ['mcp', 'add', 'codeAgent', '--', entry.command, ...entry.args],
...
/**
 * Install codeAgent skills to a target directory.
 * Each skill is installed as {targetDir}/codeAgent-{skillName}/SKILL.md
...
/**
 * Install global Cursor skills to ~/.cursor/skills/codeAgent/
 */
...
/**
 * Install global OpenCode skills to ~/.config/opencode/skill/codeAgent/
 */
...
/**
 * Install global Codex skills to ~/.agents/skills/codeAgent/
 */
...
export const setupCommand = async () => {
  ...
  console.log('    2. Run: codeAgent analyze');
