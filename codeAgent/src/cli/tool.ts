 ...
 * 
 * Exposes codeAgent tools (query, context, impact, cypher) as direct CLI commands.
 * Bypasses MCP entirely — invokes LocalBackend directly for minimal overhead.
 * 
 * Usage:
 *   codeAgent query "authentication flow"
 *   codeAgent context --name "validateUser"
 *   codeAgent impact --target "AuthService" --direction upstream
...
async function getBackend(): Promise<LocalBackend> {
  ...
  if (!ok) {
    console.error('codeAgent: No indexed repositories found. Run: codeAgent analyze');
    process.exit(1);
  }
  return _backend;
}
...
function output(data: any): void {
  ...
  try {
    ...
    if (err?.code === 'EPIPE') {
      // Consumer closed the pipe (e.g., `codeAgent cypher ... | head -1`)
...
export async function queryCommand(queryText: string, options?: {
  ...
  if (!queryText?.trim()) {
    console.error('Usage: codeAgent query <search_query>');
    process.exit(1);
  }
...
export async function contextCommand(name: string, options?: {
  ...
  if (!name?.trim() && !options?.uid) {
    console.error('Usage: codeAgent context <symbol_name> [--uid <uid>] [--file <path>]');
    process.exit(1);
  }
...
export async function impactCommand(target: string, options?: {
  ...
  if (!target?.trim()) {
    console.error('Usage: codeAgent impact <symbol_name> [--direction upstream|downstream]');
    process.exit(1);
  }
  ...
  try {
    ...
    output({
      ...
      target: { name: target },
      direction: options?.direction || 'upstream',
      suggestion: 'Try reducing --depth or using codeAgent context <symbol> as a fallback',
    });
    process.exit(1);
  }
}
...
export async function cypherCommand(query: string, options?: {
  ...
  if (!query?.trim()) {
    console.error('Usage: codeAgent cypher <cypher_query>');
    process.exit(1);
  }
