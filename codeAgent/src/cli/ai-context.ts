 ...
 * 
 * Creates AGENTS.md and CLAUDE.md with full inline codeAgent context.
...
const codeAgent_START_MARKER = '<!-- codeAgent:start -->';
const codeAgent_END_MARKER = '<!-- codeAgent:end -->';

/**
 * Generate the full codeAgent context content.
 *
 ...
 */
function generatecodeAgentContent(projectName: string, stats: RepoStats, generatedSkills?: GeneratedSkillInfo[]): string {
...
| Index, status, clean, wiki CLI commands | \`.claude/skills/codeAgent/codeAgent-cli/SKILL.md\` |${generatedRows ? '\n' + generatedRows : ''}`;

  return `${codeAgent_START_MARKER}
# codeAgent — Code Intelligence

This project is indexed by codeAgent as **${projectName}** (${stats.nodes || 0} symbols, ${stats.edges || 0} relationships, ${stats.processes || 0} execution flows). Use the codeAgent MCP tools to understand code, assess impact, and navigate safely.
...
3. \`READ codeAgent://repo/${projectName}/process/{processName}\` — trace the full execution flow step by step
...
| Resource | Use for |
|----------|---------|
| \`codeAgent://repo/${projectName}/context\` | Codebase overview, check index freshness |
| \`codeAgent://repo/${projectName}/clusters\` | All functional areas |
| \`codeAgent://repo/${projectName}/processes\` | All execution flows |
| \`codeAgent://repo/${projectName}/process/{name}\` | Step-by-step execution trace |
...
/**
 * Create or update codeAgent section in a file
 * - If file doesn't exist: create with codeAgent content
 * - If file exists without codeAgent section: append
 * - If file exists with codeAgent section: replace that section
 */
async function upsertcodeAgentSection(
  filePath: string,
  content: string
  ...
  // Check if codeAgent section already exists
  const startIdx = existingContent.indexOf(codeAgent_START_MARKER);
  const endIdx = existingContent.indexOf(codeAgent_END_MARKER);

  if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
    // Replace existing section
    const before = existingContent.substring(0, startIdx);
    const after = existingContent.substring(endIdx + codeAgent_END_MARKER.length);
...
/**
 * Install codeAgent skills to .claude/skills/codeAgent/
 * Works natively with Claude Code, Cursor, and GitHub Copilot
 */
async function installSkills(repoPath: string): Promise<string[]> {
  const skillsDir = path.join(repoPath, '.claude', 'skills', 'codeAgent');
  ...
  const skills = [
    ...
    {
      name: 'codeAgent-debugging',
      description: 'Use when the user is debugging a bug, tracing an error, or asking why something fails. Examples: "Why is X failing?", "Where does this error come from?", "Trace this bug"',
    },
    {
      name: 'codeAgent-impact-analysis',
      description: 'Use when the user wants to know what will break if they change something, or needs safety analysis before editing code. Examples: "Is it safe to change X?", "What depends on this?", "What will break?"',
    },
    {
      name: 'codeAgent-refactoring',
    ...
    {
      name: 'codeAgent-cli',
      description: 'Use when the user needs to run codeAgent CLI commands like analyze/index a repo, check status, clean the index, generate a wiki, or list indexed repos. Examples: "Index this repo", "Reanalyze the codebase", "Generate a wiki"',
    },
  ];
...
Use codeAgent tools to accomplish this task.
`;
      }
...
export async function generateAIContextFiles(
  ...
  const content = generatecodeAgentContent(projectName, stats, generatedSkills);
  const createdFiles: string[] = [];

  // Create AGENTS.md (standard for Cursor, Windsurf, OpenCode, Cline, etc.)
  const agentsPath = path.join(repoPath, 'AGENTS.md');
  const agentsResult = await upsertcodeAgentSection(agentsPath, content);
  createdFiles.push(`AGENTS.md (${agentsResult})`);

  // Create CLAUDE.md (for Claude Code)
  const claudePath = path.join(repoPath, 'CLAUDE.md');
  const claudeResult = await upsertcodeAgentSection(claudePath, content);
  createdFiles.push(`CLAUDE.md (${claudeResult})`);

  // Install skills to .claude/skills/codeAgent/
  const installedSkills = await installSkills(repoPath);
  if (installedSkills.length > 0) {
    createdFiles.push(`.claude/skills/codeAgent/ (${installedSkills.length} skills)`);
  }
