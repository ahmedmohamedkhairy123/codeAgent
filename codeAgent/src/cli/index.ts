...
program
  .name('codeAgent')
  .description('codeAgent local CLI and MCP server')
  .version(pkg.version);
...
program
  ...
  .option('--skills', 'Generate repo-specific skill files from detected communities')
   ...
   .addHelpText('after', '\nEnvironment variables:\n  codeAgent_NO_GITIGNORE=1  Skip .gitignore parsing (still reads .codeAgentignore)')
...
program
  .command('clean')
  .description('Delete codeAgent index for current repo')
...
program
  ...
  .option('--api-key <key>', 'LLM API key (saved to ~/.codeAgent/config.json)')
