...
// NOTE: Negation patterns in .codeAgentignore (e.g. `!vendor/`) cannot override
...
/**
 * Load .gitignore and .codeAgentignore rules from the repo root.
 * Returns an `ignore` instance with all patterns, or null if no files found.
 */
export interface IgnoreOptions {
  /** Skip .gitignore parsing, only read .codeAgentignore. Defaults to codeAgent_NO_GITIGNORE env var. */
  noGitignore?: boolean;
}
...
export const loadIgnoreRules = async (
  ...
  const skipGitignore = options?.noGitignore ?? !!process.env.codeAgent_NO_GITIGNORE;
  const filenames = skipGitignore
    ? ['.codeAgentignore']
    : ['.gitignore', '.codeAgentignore'];
 ...
 * - .gitignore / .codeAgentignore patterns (via `ignore` package)
...
export const createIgnoreFilter = async (repoPath: string, options?: IgnoreOptions) => {
  ...
  return {
    ignored(p: Path): boolean {
      ...
      if (!rel) return false;
      // Check .gitignore / .codeAgentignore patterns
    ...
    childrenIgnored(p: Path): boolean {
      ...
      // Check against .gitignore / .codeAgentignore patterns.
