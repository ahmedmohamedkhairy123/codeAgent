 ...
 * Repository Manager
 * 
 * Manages codeAgent index storage in .codeAgent/ at repo root.
 * Also maintains a global registry at ~/.codeAgent/registry.json
...
/**
 * Shape of an entry in the global registry (~/.codeAgent/registry.json)
 */
...
const codeAgent_DIR = '.codeAgent';

// ─── Local Storage Helpers ─────────────────────────────────────────────

/**
 * Get the .codeAgent storage path for a repository
 */
export const getStoragePath = (repoPath: string): string => {
  return path.join(path.resolve(repoPath), codeAgent_DIR);
};
 ...
 *
 * Returns:
 *   found        — true if .codeAgent/kuzu existed and was deleted
...
/**
 * Check if a path has a codeAgent index
 */
...
/**
 * Find .codeAgent by walking up from a starting path
 */
...
/**
 * Add .codeAgent to .gitignore if not already present
 */
export const addToGitignore = async (repoPath: string): Promise<void> => {
  ...
  try {
    ...
    if (content.includes(codeAgent_DIR)) return;
    
    const newContent = content.endsWith('\n') 
      ? `${content}${codeAgent_DIR}\n`
      : `${content}\n${codeAgent_DIR}\n`;
    await fs.writeFile(gitignorePath, newContent, 'utf-8');
  } catch {
    // .gitignore doesn't exist, create it
    await fs.writeFile(gitignorePath, `${codeAgent_DIR}\n`, 'utf-8');
  }
};

// ─── Global Registry (~/.codeAgent/registry.json) ───────────────────────

/**
 * Get the path to the global codeAgent directory
 */
export const getGlobalDir = (): string => {
  return path.join(os.homedir(), '.codeAgent');
};
 ...
 * Called after `codeAgent analyze` completes.
 */
...
// ─── Global CLI Config (~/.codeAgent/config.json) ─────────────────────────
...
/**
 * Load CLI config from ~/.codeAgent/config.json
 */
...
/**
 * Save CLI config to ~/.codeAgent/config.json
 */
