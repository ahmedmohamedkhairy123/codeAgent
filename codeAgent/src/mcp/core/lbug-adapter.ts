 ...
 * Retries on lock errors (e.g., when `codeAgent analyze` is running).
 *
...
async function doInitLbug(repoId: string, dbPath: string): Promise<void> {
  ...
  try {
    ...
    throw new Error(`LadybugDB not found at ${dbPath}. Run: codeAgent analyze`);
  }
  ...
  if (!shared) {
    ...
    // avoids lock conflicts when `codeAgent analyze` is writing.
