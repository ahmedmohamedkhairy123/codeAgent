 ...
 * 
 * Defines the tools that codeAgent exposes to external AI agents.
...
export const codeAgent_TOOLS: ToolDefinition[] = [
  {
    name: 'list_repos',
    description: `List all indexed repositories available to codeAgent.

Returns each repo's name, path, indexed date, last commit, and stats.

WHEN TO USE: First step when multiple repos are indexed, or to discover available repos.
AFTER THIS: READ codeAgent://repo/{name}/context for the repo you want to work with.
...
WHEN TO USE: Complex structural queries that search/explore can't answer. READ codeAgent://repo/{name}/schema first for the full schema.
...
AFTER THIS: Use impact() if planning changes, or READ codeAgent://repo/{name}/process/{processName} for full execution trace.
...
AFTER THIS: Review affected processes. Use context() on high-risk symbols. READ codeAgent://repo/{name}/process/{name} for full traces.
