 ...
 * 
 * Provides tool implementations using local .codeAgent/ indexes.
...
// AI context generation is CLI-only (codeAgent analyze)
...
function logQueryError(context: string, err: unknown): void {
  ...
  console.error(`codeAgent [${context}]: ${msg}`);
}
...
export class LocalBackend {
  ...
  private async refreshRepos(): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (kuzu.found && kuzu.needsReindex) {
        console.error(`codeAgent: "${entry.name}" has a stale KuzuDB index. Run: codeAgent analyze ${entry.path}`);
      }
  ...
  async resolveRepo(repoParam?: string): Promise<RepoHandle> {
    ...
    if (this.repos.size === 0) {
      throw new Error('No indexed repositories. Run: codeAgent analyze');
    }
    if (repoParam) {
  ...
  private async bm25Search(repo: RepoHandle, query: string, limit: number): Promise<any[]> {
    ...
    try {
      ...
      console.error('codeAgent: BM25/FTS search failed (FTS indexes may not exist) -', err.message);
      return [];
    }
  ...
  private async impact(repo: RepoHandle, params: {
    ...
    try {
      ...
      return {
        ...
        impactedCount: 0,
        risk: 'UNKNOWN',
        suggestion: 'The graph query failed — try codeAgent context <symbol> as a fallback',
      };
    }
  }
