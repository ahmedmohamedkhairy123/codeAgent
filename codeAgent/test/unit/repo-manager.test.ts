...
describe('getStoragePath', () => {
  it('appends .codeAgent to resolved repo path', () => {
    const result = getStoragePath('/home/user/project');
    expect(result).toContain('.codeAgent');
    expect(path.basename(result)).toBe('.codeAgent');
  });
...
describe('getStoragePaths', () => {
  it('returns storagePath, lbugPath, metaPath', () => {
    ...
    expect(paths.storagePath).toContain('.codeAgent');
...
describe('readRegistry', () => {
  it('returns empty array when registry does not exist', async () => {
    // readRegistry reads from ~/.codeAgent/registry.json
...
describe('saveCLIConfig / loadCLIConfig', () => {
  ...
  beforeEach(async () => {
    tmpHandle = await createTempDir('codeAgent-config-test-');
