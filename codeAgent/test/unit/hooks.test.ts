 ...
 *
 * Tests the hook scripts (codeAgent-hook.cjs and codeAgent-hook.js) that run
 * as PreToolUse and PostToolUse hooks in Claude Code.
 *
 * Covers:
 * - extractPattern: pattern extraction from Grep/Glob/Bash tool inputs
 * - findcodeAgentDir: .codeAgent directory discovery
...
// ─── Test fixtures: temporary .codeAgent directory ───────────────────

let tmpDir: string;
let codeAgentDir: string;

beforeAll(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codeAgent-hook-test-'));
...
describe('Windows .cmd extension handling', () => {
  ...
  it('Plugin hook uses .cmd extension for Windows codeAgent binary', () => {
    const source = fs.readFileSync(PLUGIN_HOOK, 'utf-8');
    expect(source).toContain("codeAgent.cmd");
  });
});
