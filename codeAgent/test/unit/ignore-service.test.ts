...
describe('loadIgnoreRules', () => {
  ...
  it('parses .codeAgentignore file', async () => {
    await fs.writeFile(path.join(tmpDir, '.codeAgentignore'), 'vendor/\n*.test.ts\n');
    const ig = await loadIgnoreRules(tmpDir);
    expect(ig).not.toBeNull();
    ...
    await fs.unlink(path.join(tmpDir, '.codeAgentignore'));
  });
...
describe('loadIgnoreRules — codeAgent_NO_GITIGNORE env var', () => {
  let tmpDir: string;
  ...
  it('skips .gitignore when codeAgent_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.gitignore'), 'data/\n');
  ...
  it('still reads .codeAgentignore when codeAgent_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.codeAgentignore'), 'vendor/\n');
