 ...
 *
 * Tests: codeAgent_TOOLS from tools.ts
...
import { codeAgent_TOOLS, type ToolDefinition } from '../../src/mcp/tools.js';

describe('codeAgent_TOOLS', () => {
  it('exports exactly 7 tools', () => {
    expect(codeAgent_TOOLS).toHaveLength(7);
  });

  it('contains all expected tool names', () => {
    const names = codeAgent_TOOLS.map(t => t.name);
  ...
  it('each tool has name, description, and inputSchema', () => {
    for (const tool of codeAgent_TOOLS) {
  ...
  it('query tool requires "query" parameter', () => {
    const queryTool = codeAgent_TOOLS.find(t => t.name === 'query')!;
