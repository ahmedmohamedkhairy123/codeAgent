 ...
 * - analyzeCommand: verify pipeline runs and creates .codeAgent/ output
 *
...
afterAll(() => {
  // Clean up .git/ and .codeAgent/ directories created during the test
  for (const dir of ['.git', '.codeAgent']) {
...
describe('CLI end-to-end', () => {
  ...
  it('analyze command runs pipeline on mini-repo', () => {
    ...
    // Successful analyze should create .codeAgent/ output directory
    const codeAgentDir = path.join(MINI_REPO, '.codeAgent');
    expect(fs.existsSync(codeAgentDir)).toBe(true);
    expect(fs.statSync(codeAgentDir).isDirectory()).toBe(true);
  });
  ...
  describe('unhappy path', () => {
    ...
    it('shows help with --help flag', () => {
      ...
      expect(result.stdout).toMatch(/codeAgent/i);
  ...
  describe('CLI error handling', () => {
    ...
    it('status on non-indexed repo reports not indexed', () => {
      ...
      // finds the parent project's .codeAgent. Use an isolated temp git
      // repo to guarantee no .codeAgent exists anywhere in the path.
  ...
  // Requires analyze to have run first (the analyze test above populates .codeAgent/).
    ...
    it('READY signal appears on stdout, not stderr', () => {
      return new Promise<void>((resolve, reject) => {
        ...
        child.stdout.on('data', (chunk: Buffer) => {
          ...
          if (stdoutBuffer.includes('codeAgent_EVAL_SERVER_READY:')) {
            foundOnStdout = true;
            child.kill('SIGTERM');
          }
        });
        ...
        child.stderr.on('data', (chunk: Buffer) => {
          ...
          if (text.includes('codeAgent_EVAL_SERVER_READY:')) {
            foundOnStderr = true;
            child.kill('SIGTERM');
          }
        });
