 ...
 *
 * Tests the hook scripts with real git repos and .codeAgent directories.
...
const CJS_HOOK = path.resolve(__dirname, '..', '..', 'hooks', 'claude', 'codeAgent-hook.cjs');
const PLUGIN_HOOK = path.resolve(__dirname, '..', '..', '..', 'codeAgent-claude-plugin', 'hooks', 'codeAgent-hook.js');
...
// ─── Temp git repo with .codeAgent ───────────────────────────────────

let tmpDir: string;
let codeAgentDir: string;

beforeAll(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hooks-e2e-'));
  codeAgentDir = path.join(tmpDir, '.codeAgent');
...
describe.each(HOOKS)('hooks e2e ($name)', ({ name, path: hookPath }) => {
  describe('PostToolUse staleness detection', () => {
    it('detects stale index when meta.json lastCommit differs from HEAD', () => {
      ...
      fs.writeFileSync(
        path.join(codeAgentDir, 'meta.json'),
      ...
      expect(output!.additionalContext).toContain('npx codeAgent analyze');
    });
    ...
    it('treats missing meta.json as stale', () => {
      // Remove meta.json
      const metaPath = path.join(codeAgentDir, 'meta.json');
  ...
  describe('PreToolUse — silent without codeAgent CLI', () => {
    // PreToolUse tries to spawn `codeAgent augment` which won't be available in CI.
    ...
    it('handles Grep pattern gracefully when CLI is unavailable', () => {
      ...
      // spawned `codeAgent augment` hangs and the 10s timeout kills the process.
  ...
  describe('directory without .codeAgent', () => {
    // The hook walks up 5 parent directories looking for .codeAgent.
    // To guarantee none is found, create a deeply nested temp dir at the
    // filesystem root where no .codeAgent could exist in any ancestor.
    let nocodeAgentDir: string;

    beforeAll(() => {
      // Use a root-level temp path so parent traversal can't find .codeAgent
      const root = os.platform() === 'win32' ? 'C:\\' : '/tmp';
      const base = path.join(root, `no-codeAgent-${Date.now()}`);
      // Nest 6 levels deep (hook walks up 5) to ensure isolation
      nocodeAgentDir = path.join(base, 'a', 'b', 'c', 'd', 'e', 'f');
      fs.mkdirSync(nocodeAgentDir, { recursive: true });
      spawnSync('git', ['init'], { cwd: nocodeAgentDir, stdio: 'pipe' });
    });

    afterAll(() => {
      // Clean up from the base directory
      const root = os.platform() === 'win32' ? 'C:\\' : '/tmp';
      const base = path.join(root, path.basename(path.resolve(nocodeAgentDir, '..', '..', '..', '..', '..', '..')));
      fs.rmSync(base, { recursive: true, force: true });
    });

    it('ignores PostToolUse when no .codeAgent directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PostToolUse',
        tool_name: 'Bash',
        tool_input: { command: 'git commit -m "x"' },
    ...
    it('ignores PreToolUse when no .codeAgent directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PreToolUse',
        tool_name: 'Grep',
