...
describe('setupCommand skills integration', () => {
  ...
  it('installs packaged, flat-file, and directory skills into cursor skills directory', async () => {
    ...
    expect(skillDirs).toContain('codeAgent-cli');

    const skillContent = await fs.readFile(
      path.join(cursorSkillsRoot, 'codeAgent-cli', 'SKILL.md'),
      'utf-8',
    );
    expect(skillContent).toContain('codeAgent CLI Commands');
  ...
  it('falls back to Codex config.toml and installs skills into ~/.agents/skills when codex CLI is unavailable', async () => {
    ...
    expect(codexConfig).toContain('[mcp_servers.codeAgent]');
    expect(codexConfig).toContain('codeAgent@latest');

    const codexSkill = await fs.readFile(
      path.join(tempHome, '.agents', 'skills', 'codeAgent-cli', 'SKILL.md'),
      'utf-8',
    );
    expect(codexSkill).toContain('codeAgent CLI Commands');
  });
  ...
  it('does not duplicate the Codex MCP section on repeated fallback setup runs', async () => {
    ...
    const sectionMatches = codexConfig.match(/\[mcp_servers\.codeAgent\]/g) ?? [];
