...
describe('filesystem-walker', () => {
  ...
  describe('.codeAgentignore support', () => {
    ...
    beforeAll(async () => {
      ...
      // Only .codeAgentignore, no .gitignore
      await fs.writeFile(path.join(nexusignoreDir, '.codeAgentignore'), 'local/\n');
    });
    ...
    it('excludes directories listed in .codeAgentignore', async () => {
  ...
  describe('combined .gitignore + .codeAgentignore', () => {
    ...
    beforeAll(async () => {
      ...
      await fs.writeFile(path.join(combinedDir, '.codeAgentignore'), 'local/\n');
    });
  ...
  describe('codeAgent_NO_GITIGNORE env var', () => {
    let envDir: string;
    ...
    it('includes gitignored directory when codeAgent_NO_GITIGNORE is set', async () => {
      const original = process.env.codeAgent_NO_GITIGNORE;
      process.env.codeAgent_NO_GITIGNORE = '1';
      try {
        const files = await walkRepositoryPaths(envDir);
        const paths = files.map(f => f.path.replace(/\\/g, '/'));
        ...
        if (original === undefined) {
          delete process.env.codeAgent_NO_GITIGNORE;
        } else {
          process.env.codeAgent_NO_GITIGNORE = original;
        }
      }
    });
  });
